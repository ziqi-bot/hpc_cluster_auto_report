...
def generate_report():
    ...
    print("Step 2: Generating cluster info")
    ...
    summary = generate_summary(df,cluster_info,dept_usage_summary)
    print(f"Generated summary: {summary}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # 保存摘要到文件
    summary_file_path = f"/data/researchHome/pdou/hpc_cluster_report/reports/summary_{timestamp}.txt"
    with open(summary_file_path, "w") as f:
        f.write(summary)
    print(f"Summary saved to: {summary_file_path}")

    pdf_path = f"/data/researchHome/pdou/hpc_cluster_report/reports/report_{timestamp}.pdf"

    print("Step 5: Creating PDF")
    create_pdf_report(cluster_info, df, summary, pdf_path,module_versions)
    print("PDF creation process complete.")
